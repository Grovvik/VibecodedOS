...
static void shell_auto_run(i32 argc, char** argv) {
    ...
    *ap = 0;
    ...
    if (is_path) {
        path_resolve(g_cwd, prog_name, resolved_path);
        ...
        // Local .bat
        path_resolve(g_cwd, prog_name, check_path);
        strcat(check_path, ".bat");
        if (file_exists(check_path)) {
            strcpy(resolved_path, check_path);
            run_as_bat = 1;
...
static void shell_execute(char* line) {
    ...
    // Parse optional output redirect:  cmd [args] > filename
    char* redirect_file = ((void*)0);
    char* gt = cmd_line;
    while (*gt) {
        if (*gt == '>') {
            *gt = 0;             // terminate the command part
            gt++;
            while (*gt == ' ' || *gt == '\t') gt++;
            redirect_file = gt;
            while (*gt && *gt != ' ' && *gt != '\t') gt++;
    ...
    // Open redirect file and install the universal redirect
    FILE* out_fp = ((void*)0);
    if (redirect_file && redirect_file[0]) {
        char resolved[FAT_MAX_PATH];
        path_resolve(g_cwd, redirect_file, resolved);
        out_fp = fopen(resolved, "w");
        if (!out_fp) {
            printf("shell: cannot open '%s' for writing\n", redirect_file);
            return;
        }
        set_stdout_redirect(out_fp);
    }

    // Dispatch — no special-casing needed for echo/cat
    ...
    // Restore normal output and flush/close the file
    if (out_fp) {
        set_stdout_redirect(((void*)0));
        fclose(out_fp);
    }
}
